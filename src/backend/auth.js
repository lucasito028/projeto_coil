...
export async function login(email, password) {
  ...
  try {
    const { data, error } = await supabase.from("user").select("id, email, name, description, password").eq("email", email).single();
...
export async function createUser(name, email, password, description) {
  try {
    ...
    const { data, error } = await supabase
      ...
      }).select("id, name, email, description, created_at").single();
